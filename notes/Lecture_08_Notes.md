...
- A learned C from B, so A does NOT advertise C back to B
- Prevents B from thinking A has an alternative path to C

**2. Poison Reverse**
> Instead of not advertising, send **infinity** back.
- A tells B: "My distance to C via you = ∞"
- More explicit than split horizon; faster convergence

**3. Route Poisoning**
> When a route fails, immediately flood the failed route as unreachable (cost=16) to all neighbors.
- Instead of waiting for next update cycle, actively announce the failure

**4. Holddown Timer**
> After receiving a route poisoned as unreachable, don't accept updates for that destination for a hold period.
- Prevents stale routes from being re-accepted during convergence
