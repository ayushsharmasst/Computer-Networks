...
| **Session keys** | Both sides independently compute the same symmetric key using Diffie-Hellman; nothing is transmitted |
| **Forward secrecy** | New key per session — past sessions remain secure even if private key is later leaked |
| **0-RTT resumption** | TLS 1.3 allows sending data on first packet for returning clients (replay-attack risk) |
