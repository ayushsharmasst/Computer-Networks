...
> "The root cause is that A learned C via B, but then TOLD B about that route. So when the real path to C (B→C) fails, B looks at A's stale advertisement and thinks there's an alternate path — not realizing A's route to C goes THROUGH B itself."

**The three standard fixes:**

**Fix 1 — Split Horizon:**

> "The rule is: **Never advertise a route back to the neighbor you learned it from.**"

> "Let's trace our example again. A learned that C=2 with NEXT HOP = B. So with split horizon, when A sends its routing update to B, it simply omits C from that update — it doesn't say anything about C to B."

> "B never receives 'A claims C=2.' So when B's direct link to C fails and B sets C=∞, B has no false alternative to fall back to. B's table correctly shows C as unreachable."

```
With Split Horizon:
  A's routing table: C=2, next hop=B
  A's update TO B: "I know A=0, B=1" ← C is silently omitted!
  
  When B-C fails:
    B sets C=∞
    B asks A for help → A has nothing to offer (didn't advertise C to B)
    B correctly keeps C=∞ ✓
```

**Fix 2 — Poison Reverse:**

> "This is split horizon with an active signal instead of silence. Instead of omitting C from A's update to B, A explicitly sends: 'C = ∞' to B."

> "Even though A's own table says C=2 via B, A LIES to B and says 'I can't reach C.' This explicitly tells B 'I cannot reach C by any path that doesn't go through you — so if you've lost C, don't count on me.'"

```
With Poison Reverse:
  A's update TO B: "A=0, B=1, C=∞"  ← actively poisons the route back
  
  When B-C fails:
    B already had "A says C=∞" cached → immediately knows no alternate path
    B converges instantly ✓
```

> "Poison Reverse converges slightly faster than split horizon alone because B doesn't need to wait — it has already received the poison. The tradeoff is slightly larger routing updates."

**Fix 3 — Route Poisoning (Triggered Updates):**

> "When B detects that the B-C link has failed, instead of waiting for the next 30-second update timer, B immediately sends an update to ALL neighbors: 'C = ∞.' This is called a triggered update."

> "This dramatically reduces the window during which count-to-infinity can occur, because B tells A about the failure before A's next periodic update fires."

**[INSTRUCTOR: Ask — 'Which of these three fixes is the most complete solution?' Answer: All three work together. Modern RIP uses all three. Split horizon + poison reverse handles the two-router loop. Triggered updates reduce convergence time. But for large networks with complex topologies, none of these are sufficient — which is why OSPF exists.]**
