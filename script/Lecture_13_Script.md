...
> "**Certificate Verification:** The browser validates the certificate chain. It checks that the certificate is issued to www.example.com, is not expired, and is signed by a CA that the OS trusts. This is how you know you're talking to the REAL example.com and not a man-in-the-middle. If validation fails: 'Your connection is not private.'"

> "**Session Keys and Diffie-Hellman:** Here's the magic. How do two parties end up with the same encryption key without ever sending it to each other? If they sent the key over the wire, anyone watching could see it."

**[INSTRUCTOR: Walk through this carefully. It's the single most-asked concept in TLS.]**

> "Diffie-Hellman Key Exchange — the paint mixing analogy. Imagine Alice and Bob want a shared secret color of paint, but Eve is watching everything."

Draw this on the board:

```
Public starting color:  YELLOW (everyone knows this)
Alice's secret:         RED  (never revealed)
Bob's secret:           BLUE (never revealed)

1. Alice mixes: Yellow + Red = ORANGE  → sends ORANGE to Bob
2. Bob mixes:   Yellow + Blue = GREEN  → sends GREEN to Alice

Eve sees: Yellow, Orange, Green — but cannot determine Red or Blue.

3. Alice takes Bob's GREEN and mixes in her secret RED:
     Green + Red = BROWN (= Yellow + Blue + Red)

4. Bob takes Alice's ORANGE and mixes in his secret BLUE:
     Orange + Blue = BROWN (= Yellow + Red + Blue)

Both have BROWN. Neither sent BROWN over the wire. Eve cannot get BROWN.
```

> "The actual math uses exponentiation modulo a prime, not paint — but the principle is identical. Alice picks a private number 'a', computes g^a mod p and sends it. Bob picks 'b', computes g^b mod p and sends it. Alice computes (g^b)^a = g^ab. Bob computes (g^a)^b = g^ab. Both get the same g^ab. No one who watched the exchange can compute g^ab without knowing either a or b — this is called the Discrete Logarithm Problem and has no known efficient solution."

> "This g^ab becomes the shared secret from which AES session keys are derived (through a key derivation function). Every session uses a fresh a and b — so even if somehow someone cracked one session's key, previous sessions remain safe. This is **forward secrecy**."

> "**TLS 1.3 improvement:** TLS 1.2 required 2 full round trips for the handshake. TLS 1.3 embedded the Diffie-Hellman key share in the first message (ClientHello), making it just 1-RTT. For returning clients, TLS 1.3 supports **0-RTT** — the server sends a session ticket on the first visit; the client uses it to send encrypted data immediately on the next visit, saving the handshake entirely. (0-RTT has replay-attack risk for non-idempotent requests.)"
